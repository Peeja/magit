...
are highlighted."
  (if (and (not selection) ; TODO handle selection properly
           (magit-section-match 'commit section)
           (magit-section-children section))
      (progn (magit-diff-highlight-list section) t)
...
(defun magit-diff-highlight-recursive (section &optional selection)
  (if (magit-section-match '(module-commit commit) section)
...
(defun magit-diff-highlight-list (section &optional selection face)
